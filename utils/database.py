...
class Database:
    ...
    def get_config_id(self, info_hash):
        self.cursor.execute("SELECT config_id FROM tasks WHERE info_hash = ? ORDER BY id DESC LIMIT 1", (info_hash,))
        return self.cursor.fetchone()

    def set_task_done(self, info_hash):
        self.cursor.execute("UPDATE tasks SET done = 1 WHERE info_hash = ?", (info_hash,))


if __name__ == '__main__':
    d = Database()
    print(d.get_config_id("46487a6d4ccf6f4c5e31843cc13fa74849282587"))